...
<%@ page isErrorPage="false" errorPage="error.jsp"
	import="java.util.Date,java.text.DateFormat,chatroom.services.*"%>
<%@ page import="java.util.HashMap"%>
<%@ page import="java.util.Iterator"%>
<%@ page import="java.util.Map"%>
<%@ page import="java.util.Set"%>
<%@ page import="chatroom.helper.ChatRoomList,chatroom.helper.ChatRoom,chatroom.helper.Chatter,chatroom.helper.Message"%>
...
<%
	...
	if (nickname != null) {
		try {
			roomList = (ChatRoomList) application
			...
			if (roonName != null && roonName != "") {
				...
				if (chatRoom != null) {
					...
					if (enteredAt != -1) {
						...
						messages = chatRoom.getMessages(chatter
								.getLoginTime());
					}
			...
			throw new ServletException(
					"Unable to get handle to ServletContext");
		}
%>
<HTML>
<HEAD>
	...
	function winopen(path) {
		chatterinfo = window
				.open(
						path,
						"chatterwin",
						"scrollbars=no,resizable=yes, width=400, height=300, location=no, toolbar=no, status=no");
		chatterinfo.focus();
	}
//-->
</script>
...
<BODY onLoad="window.location.hash='#current'" bgcolor="#FFFFFF">
	...
	<table width="100%" border="0">
		<tr>
			<td width="70%" valign="top"><%@ include file="header.jsp"%>
				<table>
					<tr>
						<td>
							<h3>
								<i><%=(String) session.getAttribute("nickname")%></i> you are in
								room <b><%=roonName%></b>
							</h3> <%
 	if (messages != null && messages.length > 0)
 		{
 			for (int i = 0; i < messages.length; i++)
 			{
 				...
 				{
 					out.write("<font face=\"Arial\" size=\"2\" color=\"blue\"><b>" + chatterName + " (" + DateFormat.getTimeInstance().format(date) + ")&gt;</b></font> "
 							+ strmsg + "<br>\n");
 				}
					...
					<tr>
						<td>
							<table width="100%" border="0" cellpadding="0" cellspacing="0"
								bgcolor="#2C259C">
								<tr>
									<td><span class="white"><%=chatRoom.getNoOfChatters()%>
											people in this room</span><br></td>
								</tr>
							</table> <%
 ...
 %> <font face="Arial" size="2" color="blue"><%=chatters[i].getName() + " (" + chatters[i].getSex() + ")<br>"%></font>
							<%
										...
										{
											out.write("<font face=\"Arial\" size=\"2\"><a href=\"javascript:winopen('viewinfo.jsp?chatterName=" + chatters[i].getName()
													+ "')\" title=\"View information about " + chatters[i].getName() + "\">" + chatters[i].getName() + "</a> (" + chatters[i].getSex() + ")</font><br>");
										}
									}
